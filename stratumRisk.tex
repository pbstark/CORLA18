...
fully counted stratum (call the stratum $s$), and continue to audit in the other, 
but against the overall margin, adjusted for the ``known'' tally in the stratum that had 
been counted: we test against the share $V_{w\ell} - A_{w\ell,s} \equiv \lambda_t' V_{w\ell}$, rather than 
against the share $\lambda_t V_{w\ell}$.
...
finds.
However, if the hypothesis changes, there's only one value possible for $\lambda_t'$---which
...
\begin{itemize}
   ...
   \item the hypothesis is not rejected in stratum $s$; the threshold $\lambda_t$
...
Consider the conditional probability that a test would reject the hypothesis that the margin is less than 
$\lambda_t' V_{w\ell}$, given that the other stratum, $s$, is tabulated by hand.
Because the tests in the two strata are statistically independent, conditioning on an event in stratum $s$
does not affect the probability of rejection in stratum $t$.
That is, the conditional probability of rejecting the null hypothesis for stratum $t$ 
is equal to the corresponding unconditional probability.
The chance that the audit will go to a full hand count in stratum $t$ is at least 
$1-\alpha_t$ if the (new) null is true (i.e., if the outcome is incorrect). 
The conditioning does not increase the overall chance of incorrectly rejecting the null.
