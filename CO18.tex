...
to simpler sampling designs.
We develop a novel method, SUITE, for conducting RLAs from stratified samples.
SUITE tests all possible partitions of outcome-changing error across strata.
It can combine arbitrary methods for finding a $P$-value for the hypothesis that the 
``overstatement error'' in a single stratum exceeds a specified threshold into a combined
$P$-value for a particular partition.
...
This approach is new and immediately useful in Colorado.
...
Extant approaches to conducting a RLA of a statewide contest would require Colorado to make major procedural changes, or would sacrifice the efficiency of ballot-level comparison audits.
We present modifications to ballot-polling and ballot-level comparison that provide the stratum-level 
$P$-values SUITE requires.
In numerical examples relevant to Colorado, SUITE is quite efficient compared to using ballot-polling throughout the state.
