...
The total number of reported votes for candidate~$w$ is $V_{w,s}$ 
and the total for candidate~$\ell$ is $V_{\ell,s}$.
Thus $V_{w,s} > V_{\ell,s}$, since $w$ is reported to have gotten more votes than $\ell$.

Let $V_{w\ell} \equiv V_{w,s} - V_{\ell,s} > 0$ denote the contest-wide margin (in votes) of 
$w$ over  $\ell$.
We have $S$ strata.
...
We want to test the compound hypothesis that $N_{\ell,s} - N_{w,s} \ge c$ against the alternative that $N_{w,s} = V_{w,s}$, $N_{\ell,s} = V_{\ell,s}$, and $N_{u,s} = V_{u,s}$, with $V_{\ell,s} - V_{w,s} < c$.
...
In this problem, $N_{u,s}$ (equivalently, $N_{w,s} + N_{\ell,s}$) is a nuisance parameter: we care about $N_{\ell,s} - N_{w,s}$.
...
$$
at $\max(W_n, L_n-c)$, $\lfloor N(W_n+L_n)/n \rfloor $, and 
$(N_s-U_n)/2$ to get the values of $f = f^+ + f^-$ at those three points, along with 
upper bounds on $f$ on the ranges 
$\{\max(W_n, L_n), \ldots, \lfloor N(W_n+L_n)/n\rfloor \}$ and
$\{\lfloor N(W_n+L_n)/n \rfloor, \ldots, (N_s-U_n)/2\}$.
At stage $j$, we have evaluated $f$, $f^+$, and $f^-$ at $j$ points $x_1 < x_2 < x_j$, and 
