...
\comment{Maybe try $\omega(\delta) \le \sup_{\lambda} | d \chi(\lambda)/d\lambda)| \delta$?}
...
When $N_{u,2}$ is not known but $\delta$ is small, this will still be linear in $c(\lambda)$. \comment{Where does this bound come from? It isn't OK just to linearize in $\delta$---that begs the question.}
