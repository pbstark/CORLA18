...
The null is true if and only if there exist \textit{some} $S$-tuple of real numbers $(\lambda_s)_{s=1}^s$ such that $\sum_s \lambda_s = 1$ and $\omega_{w\ell, s} \ge \lambda_s V_{w\ell}$ for all $s$.\footnote{%
  ``If'' is straightforward. For ``only if,'' set $\lambda_s = \frac{\omega_{w\ell, s}}{\sum_t \omega_{w\ell, t}}$.
}
...
\end{itemize}
Other functions one might use include Liptak's function, $T = \sum_i \Phi^{-1}(1-p_i)$,
and Tippett's function, $T = \max_i (1-p_i)$.

Fisher's function is convenient for this application because the tests in different strata are independent, so analytic formulas based on the chi-squared distribution can be used to calibrate the distribution of $\chi(\cdot)$. 
If the tests across strata were correlated, the distribution of the combination function would need to be calibrated by simulation; some other combining function might have better properties than Fisher's \cite{pesarinSalmaso10}.



\subsection{Maximizing Fisher's combined $P$-value for $S=2$}
We now specialize to the case $S=2$.
...
Recall that $p_s(\cdot)$ is monotonically increasing, so $p_1(\lambda)$ is
...
Those audits only covered contests restricted to a single county,\footnote{%
In some cases, only one county's portion of a cross-jurisdictional contest was audited; the resulting audit was not really a RLA.}, so counties did not need to coordinate their sampling.
...
has led many Colorado counties to purchase voting systems
that can be audited using ballot-level comparisons.
...
As mentioned above, Colorado could simply revert to ballot-polling audits for all cross-jurisdictional contests, but that would entail a loss of efficiency in CVR counties.
Alternatively, they could use batch-level comparison audits, with batches of size one in CVR counties and larger batches in non-CVR counties.
...
However, to use the method in Colorado would require major changes to Colorado's RLATool software, for reporting contest results prior to the audit, for drawing the sample, for reporting audit findings, and for determining when the audit can stop. 
...
for physically identifiable groups of ballots.
If a county's voting system can only report subtotals by precinct but 
the county does not sort paper ballots by
precinct, this approach might require major changes to how the county handles the ballots.
...
To use comparison audits in one or more strata, previous methods must be modified to test whether the overstatement error exceeds $\lambda_s V_{w\ell}$, rather than simply $V_{w\ell}$.
Appendix~\ref{sec:appendix-comparison} demonstrates this generalization for arbitrary batch sizes, although in Colorado, we would expect batches to consist of individual ballots.
The derivation explicitly considers only a single contest, but the 
...
Appendix~\ref{sec:appendix-polling} extends ballot-polling to test whether the margin exceeds an arbitrary threshold, rather than simply testing whether one candidate got more votes than another.
...
To allow the error to be partitioned across the strata via $\lambda_s$, we need to draw inferences about the \emph{difference} between the number of votes for $w$ and the number of votes for $\ell$.
