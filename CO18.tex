...
SUITE is immediately useful for RLAs in Colorado and many other states with hybrid voting systems.
...
%In Colorado, one stratum would comprise all ballots cast in CVR counties and the other all ballots cast in no-CVR counties.
...
In 87\% of 10,000 simulated stratified ''hybrid'' audits, sample sizes of 500 ballots
in the CVR stratum and 700 ballots in the no-CVR stratum
(1,200 ballots in all)
would have sufficed to confirm the outcome to a risk limit of 10\%, if the reported results were correct.
For the remaining 13\%, further expansion of the audits would have been necessary, due to an unfavorable random selection of ballots for the ballot-polling portion.
...
In 100\% of 10,000 simulations, auditing 43~ballots from the 
...
The risk limit is 5\%.
For a sample of 500~ballots from the CVR stratum and 1000~ballots from the no-CVR stratum, the maximum Fisher's combined $P$-value is over 25\%, so the audit cannot stop there.
...
Among other things, the new approach solves a problem of current interest in Colorado, by allowing ballot-polling in some counties to be combined with ballot-level comparisons in others to conduct RLAs of contests that cross jurisdictional lines, such as statewide contests and many federal contests.
...
\section{Ballot-polling audits with a margin threshold}\label{sec:appendix-polling}
