...
% add missing word; don't tether to June
=======
is not essential, it will be necessary
>>>>>>> 14ef92cab137dfbf545e00aa70232fe2428ff5e4
...
Thus, either we accept reduced % a cut to     %% somehow "a cut to efficiency" catches in my ear 
...
across both kinds of voting technology. % "mixed jurisdictional voting technology" is off-putting

This report addresses both issues, providing 
options for effectively auditing  % a handful of ways of dealing with   %% sounds almost morose 
heterogeneous 
...
% ML: it seems that this short section is here mostly to provide text between the head and the subhead.
% I don't think we should encourage readers to associate basic terms used throughout the paper with
% crude (and unpleasant) approaches. Hmmm.
...
As explained below, these stratified “hybrid” audits require the specification of some additional parameters: \lambda_1 for dividing the tolerable overstatement error up, and the strata risk limits $\{\alpha_s\}$.
...
If the audit in the CVR stratum found no errors and the tolerable overstatement error was 10\% of the margin, 
it would terminate after examining 3642 ballots. (This workload would be randomly distributed across
the CVR counties, roughly in proportion to ballots cast.)
...
did not exceed its allocation, 90\% of the margin. As always, $\lambda_1$ could be adjusted to
