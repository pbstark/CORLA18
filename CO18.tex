...
(Ballot polling and ballot-level comparison audits are described described below.) 
...
The stratified audit thus involves examining more randomly selected ballots from the strata until 
...
distribution with $2S$~degrees of freedom, or until all strata have been fully hand tabulated.

$p_s(\lambda)$ could be a $P$-value for the hypothesis
$\omega_{w\ell,s} \ge \lambda_s V_{w\ell}$ from any test procedure (although
if the audit is to be sequential, the tests in the strata must be sequential tests or
...
One could use functions other than Fisher's to combine the stratum-level $P$-values,
provided they satisfy the following properties listed by \cite{pesarinSalmaso10}:
\begin{itemize}
...
In Colorado, the two strata would consist of all ballots cast in CVR counties, and all ballots cast on non-CVR counties.
...
For a given set of audit data, finding the maximum $P$-value over all $\mathbf{\lambda}$
...
A rigorous approach is to use bounds on Fisher's combining function $\chi$ for all
$\lambda$. 
...
Recall that $p_s(\cdot)$ are monotonically increasing functions, so $p_1(\lambda)$ increases monotonically with $\lambda$ and $p_2(1-\lambda)$ decreases monotonically with $\lambda$.
...
but $\chi(a_k)$ is above this quantile at each of the grid points $\{a_k\}$, then one should improve the lower bound by refining the grid and/or by increasing the sample size in one or both strata.

\section{Strategies for auditing cross-jurisdictional contests}\label{sec:combiningMethods}

The stratified auditing method in Section~\ref{sec:stratified} makes it possible to conduct RLAs of cross-jurisdictional contests and still take advantage of ballot-level comparison methods when not all jurisdictions have voting systems that support ballot-level comparions---as is the case in Colorado, where most ballots are cast in precincts with equipment that produces CVRs, but some are not.
...
The first risk-limiting election audits under this statute were conducted in November, 2017, and the second were conducted in July, 2018.\footnote{%
 See \url{https://www.sos.state.co.us/pubs/elections/RLA/2017RLABackground.html}
}
Colorado has not yet conducted an RLA of any cross-jurisdictional contests (in some cases, they audited one county's portion of a cross-jurisdictional contest as if the contest were entirely contained in the county, but the result is not an RLA of the contest).

Under Colorado's ``uniform voting system'' program,\footnote{%
  \url{https://www.sos.state.co.us/pubs/elections/VotingSystems/UniformVotingSystem.html}
} 
many Colorado counties purchased voting systems that support ballot-level comparisons.
As of June, 2018, roughly 98\% of active Colorado voters vote in CVR counties.
...
As discussed above, Colorado could simply revert to ballot-polling audits for all cross-jurisdictional contests, but that would entail a loss of efficiency in CVR counties.
Alternatively, they could use batch-level comparison audits, with batches of size one in CVR counties and larger batches in non-CVR counties.\footnote{%
...
However, to use the method in Colorado would require major changes to Colorado's RLATool software, for reporting batch-level contest results prior to the audit, for drawing the sample, for reporting audit findings, and for determining when the audit can stop. 
The changes would include changes to data structures and to the county user interface, as well as changes to how counties conduct the audit. 
Among other things, this approach requires no-CVR counties to report vote subtotals
for physically identifiable groups of ballots before the audit starts.
And counties with voting systems that can only report subtotals by precinct
might have to make major changes to how they handle ballots, for instance, sorting all ballots by precinct.

Fortunately, the stratified audit approach of Section~\ref{sec:stratified} makes possible a``hybrid'' RLA that keeps the advantage of ballot-level comparison audits in CVR counties but does not require major changes to how no-CVR counties audit, nor major changes to RLATool. 
...
\subsection{Batch-level comparison audits of a tolerable overstatement in votes}
...
Appendix~\ref{sec:appendix-comparison} derives this generalization for arbitrary batch sizes, including batches consisting of individual ballots.
...
To use the new stratification method with ballot polling requires taking a different approach than \cite{lindemanEtal12} did: their approach tests whether $w$ got a larger \share{share} of the votes than $\ell$, but we need to test whether the margin in the stratum in exceeds a threshold (namely, $\lambda_s V_{w\ell}$).
This introduces a nuisance parameter, the number of ballots with votes for either $w$ or $\ell$.
We deal with this by maximizing the $P$-value over all possible values of the nuisance parameter.
Appendix~\ref{sec:appendix-polling} develops the test.

Unlike the test in \cite{lindemanEtal12}, which is based on Wald's Sequential Probability Ratio Test \cite{wald45}, the test is not inherently sequential; thus, to use it in an audit that can have ``escalation'' in steps, multiplicity has to be accounted for. See~\cite{stark08a,stark09a} for an approach based on Bonferroni's inequality.
...
The second example in the notebook is a hypothetical large statewide election with 
...
workflow for a hybrid stratified audit of an election with 2~million ballots cast.
The reported margin is just over $1\%$, but the reported winner
and reported loser are actually tied in both strata.  
The risk limit is 5\%.
For a sample of 500~ballots from the CVR stratum and 1000~ballots from the no-CVR stratum, the maximum Fisher's combined $P$-value is over 20\%, so the audit cannot stop at that point.
...
and to compute the upper and lower bounds on the $P$-value via Equation~\ref{eq:lowerbound}.
...
Among other things, the new approach solves a current problem in Colorado by allowing ballot-polling in some counties to be combined with ballot-level comparisons in others to conduct RLAs of contests that cross jurisdictional lines, such as statewide contests and many federal contests.

We give numerical examples relevant to Colorado, implemented in Jupyter notebooks can be modified and run with different contest sizes, margins, and risk limits to
