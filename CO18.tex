...
in published papers \cite{stark09b,stark09c,stark10d} including calculating
...
possible value of $\lambda$ (\comment{add cross reference!} see \eqref{????} for \emph{a priori} bounds on $\lambda$).
...
\beq
   \chi(\lambda) = -2(\ln p_1(\lambda)+ \ln p_2(1-\lambda))
          \ge -2(p_1(b) + p_2(1-a)) \equiv \chi_-[a,b).
\eeq
\comment{@Kellie: please double check for sign errors...I might have the endpoints reversed.}
